...
if ($transport->xpdo) {
    ...
    if (in_array($options[xPDOTransport::PACKAGE_ACTION], [xPDOTransport::ACTION_INSTALL,xPDOTransport::ACTION_UPGRADE], true)) {
        $permissions=['load'=>'Загрузка namespace mxBackup','mxbackup_view'=>'Просмотр mxBackup','mxbackup_manage'=>'Управление профилями и правилами','mxbackup_run'=>'Запуск резервного копирования','mxbackup_restore'=>'Восстановление из резервной копии'];
        ...
        if(!$policy){$policy=$modx->newObject('modAccessPolicy');$policy->fromArray(['name'=>'mxbackupDefault','description'=>'Полный доступ к mxBackup','parent'=>0,'template'=>$template->get('id'),'class'=>'','lexicon'=>'permissions'],'',true,true);}
        $data=$policy->get('data');if(!is_array($data)){$data=[];}foreach(array_keys($permissions) as $name){$data[$name]=true;}$policy->set('data',$data);$policy->save();
    }
}
return true;
